...
function CriarNaveEspacial() {
    ...
    const piloto = prompt('Digite o nome do Piloto:');
    const limiteTripulacao = Number(prompt('Digite o número máximo de pessoas na tripulação'));
    const nave = {
        nome: nome,
        piloto: piloto,
...
function AdicionarMembroNaTripulacao() {
    const id = Number(prompt('Digite o id da nave à qual você deseja adicionar um membro')) - 1;
...
function EnviarParaMissao() {
    const id = Number(prompt('Digite o id da nave à qual você deseja mandar em missão')) - 1;
    if (id < 0 || id >= NavesEspaciais.length || typeof id != 'number') {
        alert('ID de nave inválido');
        return;
    }
...
function mostraTripulação() {
    const id = Number(prompt('Digite o id da nave que você deseja consultar a tripulação')) - 1;
    if (id < 0 || id >= NavesEspaciais.length || typeof id != 'number') {
        alert('ID de nave inválido');
        return;
    }
